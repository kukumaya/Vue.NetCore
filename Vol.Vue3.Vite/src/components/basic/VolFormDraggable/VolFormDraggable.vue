<template>
  <div class="drag-container">
    ...
    <div class="drag-left">
      ...
      <draggable v-model="components" @end="end1" class="left-draggable-item"
        :group="{ name: 'componentsGroup', pull: 'clone', put: false }" animation="300" @start="onStart" :sort="false"
        :move="onMove1">
        <transition-group>
          <div :class="item.id == 1 ? 'item forbid' : 'item'" v-for="item in components" :key="item.id">
    ...
    <div class="drag-center">
      <div class="center-top">
        ...
        <el-button type="primary" size="mini" plain @click="save"><i class="el-icon-check"> </i>保存</el-button>
        <el-button type="primary" size="mini" plain @click="preview(true)"><i class="el-icon-view"> </i>预览</el-button>
        <el-button type="primary" size="mini" plain @click="download"><i class="el-icon-view"> </i>下载</el-button>
        <el-button type="primary" @click="clearItems" size="mini" plain><i class="el-icon-delete"> </i>清空</el-button>
        <a style="margin-left: 15px" href="http://v2.volcore.xyz/document/api"
      ...
      <el-scrollbar style="flex: 1">
        ...
        <el-form label-position="top">
          <draggable class="draggable-container" v-model="currentComponents" @end="end2" animation="300" :move="onMove"
              ...
              <div class="item2" :class="{ actived: index === currentIndex }" @click="itemClick(item, index)"
                ...
                <el-form-item :required="item.required" label-position="top" style="width: 100%"
                  ...
                  <el-col>
                    <!-- <div></div> -->
                    <!-- {{ item.name }} -->
                    <el-input v-if="item.type == 'text'" placeholder="请输入内容" v-model="item.value"
                      :disabled="item.readonly" size="medium"></el-input>
                    <el-input v-else-if="item.type == 'textarea'" type="textarea" v-model="item.value"
                      :disabled="item.readonly" placeholder="请输入内容"></el-input>
                    <el-date-picker v-else-if="item.type == 'date'" align="right" v-model="item.value" type="date"
                      :disabled="item.readonly" size="medium" placeholder="选择日期">
                    </el-date-picker>
                    <el-radio-group :disabled="item.readonly" v-else-if="item.type == 'radio'" v-model="item.value">
                      <el-radio v-for="item in item.data" :key="item.key" :label="item.value" :value="item.key">
                      </el-radio>
                      <!-- <el-radio :label="1">是</el-radio>
                      <el-radio :label="0">否</el-radio> -->
                    </el-radio-group>

                    <el-checkbox-group v-model="item.values" :disabled="item.readonly"
                      style="width: 100%; display: inline-block" v-else-if="item.type == 'checkbox'">
                      <el-checkbox v-for="item in item.data" :key="item.key" :label="item.value" :value="item.key">
                      </el-checkbox>
                    ...
                    <el-select style="width: 100%" :disabled="item.readonly" v-model="item.value" size="medium"
                      v-else-if="item.type == 'select'" placeholder="请选择">
                      <el-option v-for="item in item.data" :key="item.value" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                    <el-select style="width: 100%" :disabled="item.readonly" v-model="item.values" size="medium"
                      :multiple="true" v-else-if="item.type == 'selectList'" placeholder="请选择">
                      <el-option v-for="item in item.data" :key="item.value" :label="item.label" :value="item.value">
                      </el-option>
                    </el-select>
                    <el-tree-select style="width: 100%" :disabled="item.readonly" v-model="item.values" size="medium"
                      :data="item.data" :multiple="true" v-else-if="item.type == 'treeSelect'" placeholder="请选择">
                    </el-tree-select>
                    <el-cascader :disabled="item.readonly" style="width: 100%" v-else-if="item.type == 'cascader'"
                      v-model="item.values" :options="item.data" @change="() => { }"></el-cascader>
                    ...
                    <vol-upload v-else-if="item.type == 'img' || item.type == 'excel' || item.type == 'file'"
                      :fileInfo="item.fileInfo" :url="item.url" :img="item.type == 'img'" :excel="item.type == 'excel'"
                      :multiple="item.multiple" :max-size="item.maxSize" :max-file="item.maxFile"
                      :autoUpload="item.autoUpload">
                    </vol-upload>
                    <vol-wang-editor v-else-if="item.type == 'editor'" :url="item.url" v-model="item.value"
                      :height="item.height"></vol-wang-editor>

                    <vol-table v-else-if="item.type == 'table'" :url="item.url" :load-key="true" :index="item.edit"
                      :tableData="item.tableData" :columns="item.columns"
                      :buttons="item.buttons.filter((item) => item.ck)" :height="item.height" :pagination-hide="true"
                      :column-index="item.columnIndex" :ck="item.ck"></vol-table>
                    <el-button @click="model = true" v-else-if="item.type == 'box'" type="primary" size="small">{{
                      item.name }}</el-button>
                  </el-col>
                </el-form-item>
              </div>
    ...
    <div class="drag-right">
      ...
      <div class="attr" v-show="currentIndex != -1">
        ...
        <div class="attr-item" v-show="currentItem.type != 'table' && currentItem.hasOwnProperty('url')">
          ...
          <el-switch v-model="currentItem.multiple" style="width: 100%" active-color="#13ce66"
            inactive-color="rgb(165 165 165)" :active-value="true" :inactive-value="false">
          </el-switch>
          <div class="text" style="margin-top: 10px">是否自动上传</div>
          <el-switch v-model="currentItem.autoUpload" style="width: 100%" active-color="#13ce66"
            inactive-color="rgb(165 165 165)" :active-value="true" :inactive-value="false">
          </el-switch>
        </div>
        <div class="attr-item" v-show="currentItem.data">
          ...
          <!-- dicList -->
          <el-select style="width: 100%" v-model="currentItem.key" size="medium" @change="dicChange"
            placeholder="请选择数据源字典">
            <el-option v-for="item in dicList" :key="item.key" :label="item.value" :value="item.key">
            </el-option>
          </el-select>
        </div>
        <div class="attr-item" v-show="currentItem.type != 'table'">
        ...
        <div class="attr-item attr2" v-show="currentItem.type != 'table'">
          <div>
            ...
            <el-switch v-model="currentItem.required" style="width: 100%" active-color="#13ce66"
              inactive-color="rgb(165 165 165)" :active-value="true" :inactive-value="false">
            </el-switch>
          </div>
          ...
          <div>
            ...
            <el-switch v-model="currentItem.readonly" style="width: 100%" active-color="#13ce66"
              inactive-color="rgb(165 165 165)" :active-value="true" :inactive-value="false">
            </el-switch>
          </div>
        </div>
        <div v-show="currentItem.type == 'table'">
          <div class="attr-item">
            ...
            <div>
              <el-switch v-model="currentItem.tabs" style="width: 100%" active-text="是" inactive-text="否"
                active-color="#13ce66" inactive-color="rgb(165 165 165)" :active-value="true" :inactive-value="false">
              </el-switch>
            </div>
          </div>
          <div class="attr-item attr2">
            <el-button style="width: 100%" @click="openTableModel" type="primary" size="medium">table配置</el-button>
          </div>
        </div>
      </div>
    </div>
  ...
  <vol-box v-model="model" :height="300" :width="550" :lazy="true" title="弹出框">
    ...
    <template #footer>
      <div>
        <el-button type="primary" size="mini" @click="model = false"><i class="el-icon-close"></i>点击关闭</el-button>
        <el-button size="mini" @click="model = false"><i class="el-icon-close"></i>关闭</el-button>
      </div>
    </template>
  </vol-box>

  <vol-box v-model="previewModel" :height="600" :width="1300" :lazy="true" :padding="1" :close="false" title="预览">
    <preview style="height: 600px" :options="viewFormData"></preview>
  </vol-box>

  <vol-box v-model="tableModel" :height="600" :width="1300" :lazy="true" :padding="0" :title="currentItem.name">
    <template #content>
      <div style="height: 600px">
        <el-alert title="关于table配置" type="info" :closable="false" show-icon>
          此处table是对框架voltable基本操作的配置,如果需要事件触发、数据加载等更多功能，请在生成后的代码添加需要的功能，完整配置见文档<a
            href="http://v2.volcore.xyz/document/api" style="color: #1e88e5; margin-left: 9px"
            target="_blank">voltable</a>
        </el-alert>
        <div class="btns">
          <div class="btns-left">
            表格默认功能按钮：
            <el-checkbox v-for="item in currentItem.buttons" v-model="item.ck" :label="item.name" :key="item.name">{{
              item.name }}</el-checkbox>
          </div>
          <div class="btns-right">
            <el-button type="primary" size="mini" @click="openRowImport"><i class="el-icon-plus"></i>导入字段</el-button>
            <el-button type="primary" size="mini" @click="addRow"><i class="el-icon-plus"></i>添加字段</el-button>
            <el-button type="primary" size="mini" @click="delRow"><i class="el-icon-delete"></i>删除字段</el-button>
            <el-button type="primary" size="mini" @click="sortRow"><i class="el-icon-sort"></i>重新排列</el-button>
          </div>
        </div>
        <vol-table :load-key="true" :tableData="currnetTableData" :columns="currentTableOption" :height="448"
          ref="table" :index="true" :pagination-hide="true" :column-index="true" :ck="true"></vol-table>
      </div>
    </template>
    <template #footer>
      <div style="text-align: center">
        <el-button size="mini" @click="tableModel = false"><i class="el-icon-close"></i>关闭</el-button>
        <el-button type="primary" size="mini" @click="saveConfigOptions"><i class="el-icon-check"></i>保存</el-button>
      </div>
    </template>
  </vol-box>
  <!-- 导入列的选择界面 -->
  <vol-box v-model="importModel" :height="800" :width="1000" :lazy="true" :padding="0" :close="false" title="导入字段">
    <div class="header-btns">
      <el-input v-model="modelName" placeholder="请输入模型名称" style="width: 20%; margin-left: 2%"></el-input>
      <el-button type="primary" size="mini" @click="getModelColumns('DbFirst')" style="margin-left: 40%"><i
          class="el-icon-check"></i>DbFirst导入</el-button>
      <el-button type="primary" size="mini" @click="getModelColumns('CodeFirst')"><i
          class="el-icon-check"></i>CodeFirst导入</el-button>
    </div>
    <el-table :data="importModelRows">
      <el-table-column prop="DbColumnName" label="字段名称" width="200"></el-table-column>
      <el-table-column prop="ColumnDescription" label="字段描述" width="200"></el-table-column>
      <el-table-column prop="DataType" label="字段类型" width="200"></el-table-column>
      <el-table-column prop="Length" label="字段长度" width="200"></el-table-column>
    </el-table>
    <template #footer>
      <div style="text-align: center">
        <el-button size="mini" @click="importModel = false"><i class="el-icon-close"></i>关闭</el-button>
        <el-button type="primary" size="mini" @click="saveConfigOptions"><i class="el-icon-check"></i>保存</el-button>
      </div>
    </template>
  </vol-box>
...
export default {
  ...
  created() {
    ...
    this.http.post('api/Sys_Dictionary/GetBuilderDictionary', {}, false).then((x) => {
  ...
  methods: {
    getFormOptions(item) {
      ...
      if (['img', 'excel', 'file', 'editor', 'table'].indexOf(item.type) != -1) {
    ...
    getDicKeys(keys) {
      ...
      this.http.post('api/Sys_Dictionary/GetVueDictionary', keys, true).then((result) => {
        result.forEach((c) => {
    ...
    getModelColumns(modelSource) {
      if(this.modelName){
      if (modelSource == 'DbFirst') {
        this.http
          .get('/api/builder/GetDbTableColumns?tableName=' + tableName, {}, true)
          .then((x) => {
            if (!x.status) return this.$message.error(x.message)
            this.importModelRows = x.data
          })
      }
    }else{
    ...
    importRow() {
      if (this.currnetTableData.length) {
        this.currnetTableData.forEach((x) => {
          if (!x.field) {
            x.field = this.getField()
          }
        })
        this.currentItem.columns = this.currnetTableData
        this.importModel = false
      } else {
        this.$Message.error('请先添加列')
      }
    },
    addRow() {
    ...
    saveConfigOptions() {
      this.currentItem.columns = JSON.parse(JSON.stringify(this.currnetTableData))
    ...
    dicChange(key) {
      this.http.post('api/Sys_Dictionary/GetVueDictionary', [key], true).then((result) => {
